...
class loadInput:
    def __init__(self, dataset_name, data_normalization, number_of_stocks):
        ...
        elif dataset_name == 'can_new' or dataset_name == 'us_new':
            self.x = self.x[11:4220, :, :]
            self.y = self.y[11:4220, :]
            self.x_dates = self.x_dates[11:4220]
        elif dataset_name == 'ind_new' or dataset_name == 'ind' or dataset_name == 'hin_new' or dataset_name == 'hin':
            self.x = self.x[19:4000, :, :]
            self.y = self.y[19:4000, :]
            self.x_dates = self.x_dates[19:4000]
        else:            
            self.x = self.x[0:-10,:,:]
            self.y = self.y[0:-10,:]
